import React from "react";
import styles from "../../AdminStyles/AdminList.module.css";
import AdminList from "../../Components/Admin/AdminList.js";
import { Table, Badge, Button } from "antd";
import { getAllHomepage, deleteHomepage } from "../../Axios/Admin.js";
import { useNavigate } from "react-router-dom";
import EditIcon from "@mui/icons-material/Edit";
import DeleteIcon from "@mui/icons-material/Delete";
import { Link } from "react-router-dom";
import { toast } from "react-toastify";

const Homepage = () => {
  const navigate = useNavigate();
  const [data, setData] = React.useState([]);
  let token = "randomString";
  if (window !== "undefined" && window.localStorage.getItem("jwtAdmin")) {
    token = JSON.parse(window.localStorage.getItem("jwtAdmin"));
  }
  const getCoupons = () => {
    getAllHomepage(token)
      .then((res) => setData(res.data))
      .catch((err) => console.log(err));
  };
  React.useEffect(() => {
    getCoupons();
  }, []);
  const columns = [
    {
      title: "Id",
      dataIndex: "_id",
      width: 150,
      align: "center",
      render: (text) => <Link to={`/admin/home-page/${text}`}>{text}</Link>,
    },
    {
      title: "Brands",
      dataIndex: "brands",
      width: 75,
      align: "center",
      render: (text) => text.length,
    },
    {
      title: "You May Like",
      dataIndex: "youMayLike",
      width: 75,
      align: "center",
      render: (text) => text.length,
    },
    {
      title: "Trending",
      dataIndex: "trending",
      width: 75,
      align: "center",
      render: (text) => text.length,
    },
    {
      title: "Best in Cafecart",
      dataIndex: "bestInBruh",
      width: 75,
      align: "center",
      render: (text) => text.length,
    },
    {
      title: "New Arrivals",
      dataIndex: "newArrivals",
      width: 75,
      align: "center",
      render: (text) => text.length,
    },
    {
      title: "BannerImages",
      dataIndex: "bannerImages",
      width: 75,
      align: "center",
      render: (text) => text.length,
    },
    {
      title: "Delete",
      dataIndex: "_id",
      width: 75,
      align: "center",
      render: (text) => (
        <DeleteIcon
          sx={{ fontSize: 30, color: "red" }}
          onClick={(e) => {
            deleteHomepage(token, text)
              .then((res) => {
                toast.success("Deleted Succesfully");
                getCoupons();
              })
              .catch((err) => {
                toast.error("Deletion Failed!");
              });
          }}
        />
      ),
    },
  ];
  return (
    <div className={styles.overall}>
      <div className={styles.left}>
        <AdminList />
      </div>
      <div className={styles.right}>
        <div className={styles.rightInner}>
          <div className={styles.couponHeading}>
            <Badge
              count={data ? data.length : 0}
              offset={[20, 10]}
              overflowCount={999}
            >
              <div className={styles.couponLeft}>HomePage</div>
            </Badge>

            <div className={styles.couponRight}>
              {" "}
              <Button
                type="primary"
                size="large"
                onClick={(e) => navigate("/admin/home-page/new")}
              >
                Create HomePage Content
              </Button>
            </div>
          </div>
          <center>
            <Table
              columns={columns}
              dataSource={data}
              pagination={{ pageSize: 10 }}
              scroll={{ y: 1000 }}
            />
          </center>
        </div>
      </div>
    </div>
  );
};

export default Homepage;
