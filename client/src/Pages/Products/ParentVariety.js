...
import { parentVariety } from "../../Axios/Products.js";
...
const ParentVariety = () => {
  ...
  const getData = () => {
    setLoading(true);
    parentVariety(params.id1, params.id2)
      .then((res) => {
        setAns(res.data);
        setLoading(false);
      })
  ...
  return (
    <>
      ...
      >
        ...
        <Breadcrumb.Item href="">{createName(params.id2)}</Breadcrumb.Item>
      </Breadcrumb>
      ...
      {loading && <Sugar customLoading={true} />}
    </>
  );
};

export default ParentVariety;
